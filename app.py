...
with col1_3:
    ...
    with st.expander('Stats (P1)'):
        ...
        data1_ed = st.data_editor(pandas.DataFrame({k: v for k, v in data1 if k == "stats"}).filter(['stats']), key = 'd1')
...
with col2_1:
    ...
    with st.expander('Stats (P2)'):
        ...
        data2_ed = st.data_editor(pandas.DataFrame({k: v for k, v in data2 if k == "stats"}).filter(['stats']), key = 'd2')
